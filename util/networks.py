'''
@Author: Jilong Wang
@Date: 2019-01-15 12:02:27
@LastEditors: Jilong Wang
@Email: jilong.wang@watrix.ai
@LastEditTime: 2019-01-15 12:03:35
...
class SegNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(self.seg_score(x))
        return x
...
class DeconvUnitCrop(nn.Module):
    ...
    def _crop(self, x):
        ...
        if h > self.height:
            pad_t = self.height - h
        return torch.pad(x, [pad_l, pad_r, pad_t, pad_b])
...
class SegShuffle(nn.Module):
    def __init__(self, input_chs=3, filter_num=64, typ='conv'):
        ...
        # fusion
        self.conv_fusion6 = conv1x1(filter_num, filter_num, bn=self.bn)
        self.conv_fusion7 = conv1x1(filter_num, 3, bn=self.bn)
        self.seg_score = conv1x1(3, 1, bn=False, act=False)
        #self.seg_score = conv1x1(filter_num, 1, bn=self.bn, act=False)
    ...
    def forward(self, x):
        ...
        decode_logits = self.conv_fusion6(x)
        pix_embedding = self.conv_fusion7(decode_logits)
        binary_seg_pred = torch.sigmoid(self.seg_score(pix_embedding))
        ret = {
